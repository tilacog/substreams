...
type ModuleExecutor interface {
	...
	run(vals map[string][]byte, clock *pbsubstreams.Clock, cacheActivated bool, cursor string) error
...
func (e *MapperModuleExecutor) run(vals map[string][]byte, clock *pbsubstreams.Clock, cacheDisabled bool, cursor string) error {
	if !cacheDisabled {
...
func (e *StoreModuleExecutor) run(vals map[string][]byte, clock *pbsubstreams.Clock, cacheDisabled bool, cursor string) error {
	if !cacheDisabled {
		output, found, err := e.cache.Get(clock)
		if err != nil {
			zlog.Warn("failed to get output from cache", zap.Error(err))
